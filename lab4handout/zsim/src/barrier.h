...
class Barrier : public GlobAlloc {
    private:
        uint32_t maxThreads; // threads << 16 | waitingThreads
        ...
        MTRand rnd;
        Callee* sched; //FIXME: I really, really hate this organization, but I don't have time to refactor the barrier code, this is used for a callback when the phase is done
    ...
    public:
        Barrier(uint32_t _parallelThreads, Callee* _sched) : parallelThreads(_parallelThreads), rnd(0xBA77137), sched(_sched) {
            ...
            runList = (uint32_t*) gm_calloc(MAX_THREADS, sizeof(uint32_t));
            runListSize = 0;
            curThreadIdx = 0;
        ...
        void leave(uint32_t tid) {
            ...
            assert_msg(threadList[tid].state == RUNNING, "leave, tid %d, incorrect state %d", tid, threadList[tid].state);
