...
class HDF5BackendImpl : public GlobAlloc {
    private:
        ...
        bool skipStat(Stat* s) {
            return skipVectors && (dynamic_cast<VectorCounter*>(s) || dynamic_cast<ProxyVectorStat*>(s));
        }
        ...
        hid_t getBaseH5Type(Stat* s) {
            ...
            if (VectorCounter* vs = dynamic_cast<VectorCounter*>(s)) {
                size = vs->size();
            } else if (ProxyVectorStat* vps = dynamic_cast<ProxyVectorStat*>(s)) {
                size = vps->size();
            }
            if (size > 1) {
                hsize_t dims[] = {size};
    ...
    public:
            ...
            filename(_filename), rootStat(_rootStat), skipVectors(_skipVectors), sumRegularAggregates(_sumRegularAggregates)
        {
            ...
            dataBuf = (uint64_t*) gm_malloc(bufSize);
            curPtr = dataBuf;
        ...
        void dump(bool buffered) {
            ...
            assert(dataBuf + bufferedRecords*recordSize/sizeof(uint64_t) == curPtr);
