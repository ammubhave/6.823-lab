...
static void printHeartbeat(GlobSimInfo* zinfo) {
    ...
    hb << "Start time: " << ctime(&startTime);
    hb << "Heartbeat time: " << ctime(&curTime);
...
int main(int argc, char *argv[]) {
    ...
    info("Starting zsim, built " ZSIM_BUILDSTR);
    startTime = time(NULL);
    ...
    bool perProcessDir = conf.get<bool>("sim.perProcessDir", false);
    ...
    PinCmd* pinCmd = new PinCmd(&conf, configFile, outputDir, shmid);
    ...
    for (uint32_t procIdx = 0; procIdx < numProcs; procIdx++) {
        ...
        if (cpid) { //parent
            ...
            long pers = personality(((unsigned int)-1) /*returns current personality flags*/);
            if (pers == -1L || personality(pers | ADDR_NO_RANDOMIZE) == -1L) {
                perror("personality() call failed");
                panic("Could not change personality to disable address space randomization!");
                long newPers = personality(((unsigned int)-1));
                if ((newPers & ADDR_NO_RANDOMIZE) == 0) panic("personality() call was not honored! old 0x%lx new 0x%lx", pers, newPers);
            }
    ...
    while(/*!activeChildren.empty()*/ getNumChildren() > 0) {
        if (!gm_isready()) {sched_yield(); continue;} //wait till proc idx 0 initializes everyhting
