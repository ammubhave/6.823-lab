...
$logBlockSize = @ARGV[2];
$logNumRows = @ARGV[4];
$associativity = @ARGV[6];

#print "logBlockSize = $logBlockSize, logNumRows = $logNumRows, associativity = $associativity";
...
chomp($dirname);
$dirname =~ s/ /_/g;
$resultdir = $localdir . "/results_" . $dirname . "_b$logBlockSize" . "_r$logNumRows" . "_a$associativity" . "/";
...
for($i = 0; $i <  scalar(@testcases); $i=$i+1) {
    ...
    # Now execute.
    $tbegin = time();
    print "Executing: \n\ncd $execdir;/mit/6.823/Spring16/Pin2012/pin -t $pintool -b $logBlockSize -r $logNumRows -a $associativity -o $outfile -- $execfile\n\n";
    $execout = `cd $execdir;/mit/6.823/Spring16/Pin2012/pin -t $pintool -b $logBlockSize -r $logNumRows -a $associativity -o "$outfile" -- $execfile`;
